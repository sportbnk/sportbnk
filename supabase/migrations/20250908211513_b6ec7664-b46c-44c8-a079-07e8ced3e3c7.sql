...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Authenticated users can view teams" ON public.teams FOR SELECT TO authenticated USING (true)';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Authenticated users can view contacts" ON public.contacts FOR SELECT TO authenticated USING (true)';
  END IF;
END$$;
